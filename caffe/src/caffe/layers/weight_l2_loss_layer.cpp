...
#include "caffe/layers/weight_l2_loss_layer.hpp"
#include "caffe/util/math_functions.hpp"

namespace caffe {

template <typename Dtype>
void WeightL2LossLayer<Dtype>::Reshape(
  ...
  memset(y, 0, sizeof(y));
  for(int i = 0; i < hei; i ++)
	x[i] = exp( ((hei-1-i) * 1.0 / hei) * ((hei-1-i) * 1.0 / hei) );
  for(int j = 0; j < wid; j ++)
	y[j] = exp( (j * 1.0 / wid) * (j * 1.0 / wid) );
...
template <typename Dtype>
void WeightL2LossLayer<Dtype>::Forward_cpu(const vector<Blob<Dtype>*>& bottom,
...
template <typename Dtype>
void WeightL2LossLayer<Dtype>::Backward_cpu(const vector<Blob<Dtype>*>& top,
...
#ifdef CPU_ONLY
STUB_GPU(WeightL2LossLayer);
#endif

INSTANTIATE_CLASS(WeightL2LossLayer);
REGISTER_LAYER_CLASS(WeightL2Loss);
