function [curr_dat_sz, curr_lab1_sz, curr_lab2_sz] = store2hdf5_3(filename, data, label1, label2, create, startloc, chunksz)  
...
if create
    ...
    h5create(filename, '/label1', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    h5create(filename, '/label2', [lab_dims(1:end-1) Inf], 'Datatype', 'single', 'ChunkSize', [lab_dims(1:end-1) chunksz]); % width, height, channels, number 
    if ~exist('startloc','var') 
        startloc.dat=[ones(1,length(dat_dims)-1), 1];
        startloc.lab1=[ones(1,length(lab_dims)-1), 1];
        startloc.lab2=[ones(1,length(lab_dims)-1), 1];
    end 
    else  % append mode
    if ~exist('startloc','var')
        info=h5info(filename);
        prev_dat_sz=info.Datasets(1).Dataspace.Size;
        prev_lab1_sz=info.Datasets(2).Dataspace.Size;
        prev_lab2_sz=info.Datasets(3).Dataspace.Size;
        assert(prev_dat_sz(1:end-1)==dat_dims(1:end-1), 'Data dimensions must match existing dimensions in dataset');
        assert(prev_lab1_sz(1:end-1)==lab_dims(1:end-1), 'Label dimensions must match existing dimensions in dataset');
        assert(prev_lab2_sz(1:end-1)==lab_dims(1:end-1), 'Label dimensions must match existing dimensions in dataset');
        startloc.dat=[ones(1,length(dat_dims)-1), prev_dat_sz(end)+1];
        startloc.lab1=[ones(1,length(lab_dims)-1), prev_lab1_sz(end)+1];
        startloc.lab2=[ones(1,length(lab_dims)-1), prev_lab2_sz(end)+1];
    end
end

if ~isempty(data)
    h5write(filename, '/data', single(data), startloc.dat, size(data));
    h5write(filename, '/label1', single(label1), startloc.lab1, size(label1));  
    h5write(filename, '/label2', single(label2), startloc.lab2, size(label2));  
end

if nargout
    info=h5info(filename);
    curr_dat_sz=info.Datasets(1).Dataspace.Size;
    curr_lab1_sz=info.Datasets(2).Dataspace.Size;
    curr_lab2_sz=info.Datasets(3).Dataspace.Size;
end
